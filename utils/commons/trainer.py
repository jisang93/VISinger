...
class Trainer:
    def __init__(
        ...
        self.amp = amp
        self.amp_scalar = GradScaler(enabled=hparams["amp"])
    ...
    def run_single_process(self, task):
        ...
        if checkpoint is not None:
            self.restore_weights(checkpoint, hparams.get("ckpt_strict", True))
        elif self.on_gpu:
    ...
    def run_training_batch(self, batch_idx, batch):
        ...
        if self.amp:
            # Warninging: update() should only be called at the end of the iteration,
            #             after scaler.step(optimizer) has been invoked for all optimizers used this iteration.
    ...
    ####################
    def restore_weights(self, checkpoint, strict=True):
        # load model state
        task_ref = self.get_task_ref()
        checkpoint_state_dict = checkpoint['state_dict']
        if not strict:
            cur_state_dict = task_ref.state_dict()
            if len(checkpoint_state_dict) > 1:
                for module, _ in checkpoint_state_dict.items():
                    unmatched_keys = []
                    for key, param in checkpoint_state_dict[module].items():
                        key = f"{module}.{key}"
                        if key in cur_state_dict:
                            new_param = cur_state_dict[key]
                            if new_param.shape != param.shape:
                                unmatched_keys.append(key)
                                print(f"| Unmatched keys: {key}, {new_param.shape}, {param.shape}")
                    for key in unmatched_keys:
                        key = key.split(".")
                        module = key[0]
                        key = ".".join(key[1:])
                        del checkpoint_state_dict[module][key]
        for k, v in checkpoint_state_dict.items():
            getattr(task_ref, k).load_state_dict(v, strict=strict)
