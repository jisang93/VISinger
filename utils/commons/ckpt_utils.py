...
def get_all_ckpts(work_dir, steps=None):
    ...
    # Return the file name in order of steps
...
def load_ckpt(cur_model, ckpt_base_dir, model_name='model', force=True, strict=True):
    ...
    if checkpoint is not None:
        ...
        if "." in model_name:
            state_dict = state_dict[model_name.split(".")[0]]
            if len([k for k in state_dict.keys() if "." in k]) > 0:
                state_dict = {k[len(model_name.split(".")[1]) + 1:]: v for k, v in state_dict.items()
                              if k.startswith(f"{model_name.split('.')[1]}.")}
        else:
            state_dict = state_dict[model_name]
        if not strict:
            cur_model_state_dict = cur_model.state_dict()  # 현재 모델
            unmatched_keys = []
            for key, param in state_dict.items():  # 저장된 모델
                if key in cur_model_state_dict:  # 현재 모델
