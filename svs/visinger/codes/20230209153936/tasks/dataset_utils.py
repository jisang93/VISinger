...
from utils.audio.mel_processing import SpectrogramFixed
...
class VISingerDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        max_frames = hparams["max_frames"]
        wav = torch.Tensor(item["wav"][:max_frames*hparams["hop_size"]])
        spec = torch.Tensor(self.spec_fn(wav))
        print("spec: ", spec.shape)
        assert spec.shape[-1] == self.sizes[index], (spec.shape, self.sizes[index])
        # Mapping function
    ...
    def collater(self, samples):
        ...
        mels = collate_1d_or_2d([s["mel"] for s in samples], 0.0)
        mel_lengths = torch.LongTensor([s["mel"].shape[-1] for s in samples])
