...
def convert_pad_shape(pad_shape):
    l = pad_shape[::-1]  # reverse pad_shape
...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, bsz, seq_len, input):
        """Input is expected to be of size [Batch, T_len]."""
...
class RelativeEncoder(nn.Module):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout=0.,
                 window_size=4, block_length=None, pre_ln=False, gin_channels=None, **kwargs):
        super().__init__()
        ...
        if gin_channels is not None:
            self.pre_net = nn.Conv1d(gin_channels, hidden_channels, 1)
...
class ConvReluNorm(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size, n_layers, p_dropout=0.):
        super().__init__()
        self.in_channels = in_channels
        self.hidden_channels = hidden_channels
        self.out_channels = out_channels
...
class RelativeTransformerEncoder(nn.Module):
    def __init__(self, n_vocab, out_channels, hidden_channels, filter_channels, n_heads,
                 n_layers, kernel_size, p_dropout=0., window_size=4, block_length=None,
                 pre_net=True, pre_ln=True):
        super().__init__()
        ...
        if n_vocab > 0:
            self.embedding = Embedding(n_vocab, hidden_channels, padding_idx=0)
        if pre_net:
            self.pre = ConvReluNorm(hidden_channels, hidden_channels, hidden_channels,
                                    kernel_size=5, n_layers=3, p_dropout=0)
        
        self.encoder = RelativeEncoder(hidden_channels, filter_channels, n_heads, n_layers,
                                       kernel_size, p_dropout, window_size, block_length,
                                       pre_ln)
    
    def forward(self, x, x_mask=None, position="forward"):
        if self.n_vocab > 0:
            x_lengths = (x > 0).long().sum(-1)            
            x = self.embedding(x) * math.sqrt(self.hidden_channels)  # [Batch, T_len, Channels]
        else:
            x_lengths = (x.abs().sum(-1) > 0).long().sum(-1)
        x = torch.transpose(x, 1, -1)  # [Batch, Channels, T_len]
    ...
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size,
        ...
        if pre_net:
            self.pre = ConvReluNorm(hidden_channels, hidden_channels, hidden_channels, kernel_size=5,
                                    n_layers=3, p_dropout=0)
        self.decoder = RelativeEncoder(hidden_channels, filter_channels, n_heads, n_layers, kernel_size,
                                       p_dropout, window_size, block_length, pre_ln)
